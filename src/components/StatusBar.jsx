    ...
    >
          ...
          <div style={{ maxHeight: 200, overflow: "auto" }}>
            {graph.nodes.map((n) => {
              ...
              const isMin =
                ...
                d ===
                  Math.min(
                    ...Object.values(currentStep.dist).filter(
                      (v) => v !== Infinity,
                    ),
                  );
              return (
                ...
                >
                  <span
                    style={{
                      color:
                        currentStep.current === n.id ? C.accent2 : C.textDim,
                    }}
                  >
                    n{n.label}
                  </span>
      ...
      {currentStep?.visited && (
        ...
        >
          ...
          >
            VISITED ({currentStep.visited.size})
          </div>
      ...
      {currentStep?.mstEdges && currentStep.mstEdges.size > 0 && (
        ...
        >
          ...
          >
            MST EDGES ({currentStep.mstEdges.size})
          </div>
          <div style={{ maxHeight: 120, overflow: "auto" }}>
            {[...currentStep.mstEdges].map((eid2) => {
              ...
              const fn = graph.nodes.find((n) => n.id === e.from),
                tn = graph.nodes.find((n) => n.id === e.to);
              return (
                <div
                  key={eid2}
                  style={{ fontSize: 10, color: C.accent3, marginBottom: 2 }}
                >
                  {fn?.label} — {tn?.label} ({e.weight})
                </div>
              );
            })}
          </div>
      ...
      {currentStep?.negCycle && (
        ...
        >
          <div style={{ fontSize: 11, color: C.danger, fontWeight: 700 }}>
            ⚠ Negative Cycle Detected
          </div>
          <div style={{ fontSize: 10, color: C.textDim, marginTop: 4 }}>
            Distances are unreliable
          </div>
        </div>
      )}
      ...
      {currentStep?.queue !== undefined && (
        ...
        >
          ...
          >
            QUEUE ({currentStep.queue.length})
          </div>
          <div style={{ display: "flex", flexWrap: "wrap", gap: 4 }}>
            {currentStep.queue.map((nid, i) => {
              ...
              return (
                <span
                  key={i}
                  className="chip"
                  style={{ background: "rgba(0,212,255,0.1)", color: C.accent }}
                >
                  {n?.label ?? nid}
                </span>
              );
      ...
      {currentStep?.stack !== undefined && (
        ...
        >
          ...
          >
            STACK ({currentStep.stack.length})
          </div>
          <div style={{ display: "flex", flexWrap: "wrap", gap: 4 }}>
